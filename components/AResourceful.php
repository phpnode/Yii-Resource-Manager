<?php
Yii::import("packages.resources.components.*");
/**
...
class AResourceful extends CActiveRecordBehavior {
	...
	public function setAttributes($attributes) {
		...
		foreach($attributes as $name => $config) {
			...
			$this->_attributes[$name]->owner = $this->getOwner();
	...
	public function canGetProperty($name) {
		return method_exists($this,'get'.$name) || isset($this->_attributes[$name]);
	}
	...
	public function canSetProperty($name) {
		return method_exists($this,'set'.$name) || isset($this->_attributes[$name]);
	}
	/**
	...
	public function __get($name) {
		if (isset($this->_attributes[$name])) {
			if ($this->_attributes[$name]->multiple) {
				return $this->_attributes[$name]->getResources();
			}
			else {
				$resources = $this->_attributes[$name]->getResources();
	...
	public function __set($name, $value) {
		if (isset($this->_attributes[$name])) {
			return $this->_attributes[$name]->add($value);
		}
	...
	public function __unset($name) {
		if (isset($this->_attributes[$name])) {
			return $this->_attributes[$name]->delete();
		}
...
class AResourceAttribute extends CComponent {
	 ...
	 * @var string
	 */
	public $resourceClass = "AResource";
	/**
	...
	public function add($item) {
		if ($item instanceof CUploadedFile) {
			...
			$item = $class::fromUploadedFile($item); /* @var AResource $item */
		}
		$this->getResources();

		$item->ownerModel = get_class($this->owner);
		$item->ownerId = $this->owner->getPrimaryKey();
